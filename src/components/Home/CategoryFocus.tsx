...
export const CategoryFocus: React.FC = () => {
  ...
  const prevCategory = () => {
    setCurrentCategory((prev) => (prev - 1 + focusCategories.length) % focusCategories.length);
  };
                ...
                >
                  {category.features.map((feature, index) => (
                    <div key={feature} className="flex items-center">
                      ...
                      <span className="text-luxury-gray-400 text-sm">{feature}</span>
                    </div>
                  ))}
                </motion.div>
              ...
              {focusCategories.map((_, index) => (
                <motion.button
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    index === currentCategory ? 'bg-luxury-red' : 'bg-luxury-gray-600'
                  }`}
                />
              ))}
            </div>
